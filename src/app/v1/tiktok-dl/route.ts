...
export async function POST(req: NextRequest) {
	...
	await returnIsEnabled("tiktok-dl");

	const { estimated = 0.01 } = await getEstimatedCost("tiktok-dl");
	...
	try {
		const apiResponse = await fetch(
			`${process.env.TIKTOK_DL_URL ?? "http://localhost:7006/download"}`,
			{
				method: "POST",
				headers: {
		...
		const response = {
			message: "Failed to get response from TikTok Downloader.",
			funds: {
